...
public:
	...
	/// <param name="color">描画するスプライトの色</param>
	void DrawPicture(Vec2 position, float width, float height, float angle, float srcW, float srcH, int textureHandle, unsigned int color, Vec2 scale = { 1.0f, 1.0f }, bool isScroll = true);
	...
	/// <param name="textureHandle">テクスチャのハンドル</param>
	/// <param name="color">描画するスプライトの色</param>
	void DrawPicture(Vec2 position, float size, float angle, float srcW, float srcH, int textureHandle, unsigned int color, Vec2 scale = { 1.0f, 1.0f }, bool isScroll = true);
